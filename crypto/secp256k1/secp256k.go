...
func pubkeyFromSeckey(seckey []byte) []byte {
	...
	if SeckeyIsValid(seckey) != 1 {
		log.Panic("always ensure seckey is valid")
		return nil
	}

	var pubkey []byte = GeneratePublicKey(seckey) //always returns true
	if pubkey == nil {
		log.Panic("ERROR: impossible, BaseMultiply always returns true")
		return nil
	}
	if len(pubkey) != 33 {
	...
	if ret := PubkeyIsValid(pubkey); ret != 1 {
...
func GetXY(pubkey []byte) *XY {
	var pub_xy XY
	if !pub_xy.ParsePubkey(pubkey) {
		return nil
	}
	return &pub_xy
}
...
new_seckey:
	...
	if SeckeyIsValid(seckey) != 1 {
		log.Printf("generateDeterministicKeyPair, SeckeyIsValid fail")
		goto new_seckey //regen
	}

	var pubkey []byte = GeneratePublicKey(seckey)
...
//Rename SignatureChk
func VerifySignature(msg []byte, sig []byte, pubkey1 []byte) bool {
