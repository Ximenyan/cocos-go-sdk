...
func randFieldElement(c *BitCurve, rand io.Reader) (k *big.Int, err error) {
...
func GenerateKey(c *BitCurve, rand io.Reader) (priv *PrivateKey, err error) {
...
func Bytes2Key(c *BitCurve, byte_s []byte) (priv *PrivateKey, err error) {
	k := new(big.Int).SetBytes(byte_s)
	n := new(big.Int).Sub(c.N, one)
	k.Mod(k, n)
	k.Add(k, one)
	return
...
func hashToInt(hash []byte, c *BitCurve) *big.Int {
